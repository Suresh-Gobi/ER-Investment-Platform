    ...
    <div>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogContent>
          ...
          <Typography>
            Approved: {selectedProject?.landDetails?.approved ? "Yes" : "No"}
          </Typography>
          <Typography>
          ...
          <Typography>
            Location: {selectedProject?.location.coordinates.join(", ")}
          </Typography>
          <Typography>
            Soil pH Level: {selectedProject?.soilHealth.pHLevel}
          </Typography>
          <Typography>Nutrient Levels:</Typography>
          <Typography>
            - Nitrogen: {selectedProject?.soilHealth.nutrientLevel.nitrogen}
          </Typography>
          <Typography>
            - Phosphorus: {selectedProject?.soilHealth.nutrientLevel.phosphorus}
          </Typography>
          <Typography>
            - Potassium: {selectedProject?.soilHealth.nutrientLevel.potassium}
          </Typography>
          <Typography>
            Water Level: {selectedProject?.soilHealth.waterLevel}
          </Typography>
          <Typography>
            Humidity: {selectedProject?.humidity.value}{" "}
            {selectedProject?.humidity.unit}
          </Typography>
          <Typography>
            Temperature: {selectedProject?.temperature.value}{" "}
            {selectedProject?.temperature.unit}
          </Typography>
          <Typography>Farming Area: {selectedProject?.farmingArea}</Typography>
          <Typography>
            Plant Health - Biomass: {selectedProject?.plantHealth.biomass}
          </Typography>
          <Typography>
            CO2 Concentration: {selectedProject?.co2Concentration.value}{" "}
            {selectedProject?.co2Concentration.unit}
          </Typography>
          <Typography>
            H2O Concentration: {selectedProject?.h2oConcentration.value}{" "}
